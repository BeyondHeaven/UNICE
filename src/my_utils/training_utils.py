...
class PairedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, image_prep, tokenizer):
        """
        ...
        prepares the transformations and tokenizer to be applied on the data.

        Parameters:
        - dataset_folder (str): The root folder containing the dataset.
        ...
        self.caption = "exposure control"
        self.img_names = self._collect_img_names()

    def _collect_img_names(self):
        """
        Collect all jpg image names from the GT subfolders.

        Returns:
        list: A list containing the paths of all images.
        """
        img_names = []
        subdatasets = ["AODRAW", "fivek", "HDRP", "PASCAL-RAW", "PPR10K", "RAISE", "RAW-NOD"]

        for subdataset in subdatasets:
            folder_path = os.path.join(self.dataset_folder, 'fake_HDR', subdataset, '0', '0.2')
            if os.path.isdir(folder_path):
                img_names.extend([os.path.join(subdataset, img) for img in os.listdir(folder_path) if img.endswith('.jpg')])
        return img_names
    ...
    def __getitem__(self, idx):
        ...
        its corresponding output image, and the tokenized caption.
        ...
        subdataset, img_name = os.path.split(img_name)

        fake_HDR_root = os.path.join(self.dataset_folder, 'fake_HDR', subdataset)
        GT_root = os.path.join(self.dataset_folder, 'GT', subdataset)
        real_expo_options = ['0', '+1', '-1', '+2', '-2', '+3', '-3']
        fake_expo_options = ['0.2', '0.5', '0.8', 'input']

        # Get GT image tensor
        output_img_path = os.path.join(GT_root, img_name)
        output_img = Image.open(output_img_path).convert("RGB")
        output_t = self.T(output_img)
        output_t = F.to_tensor(output_t)
        output_t = F.normalize(output_t, mean=[0.5], std=[0.5])

        # Get fake_HDR tensors
        chosen_real_expo = random.choice(real_expo_options)
        fake_HDR_images = []
        for expo in fake_expo_options:
            img_path = os.path.join(fake_HDR_root, chosen_real_expo, expo, img_name)
            img = Image.open(img_path).convert("RGB")
            img_t = self.T(img)
            img_t = F.to_tensor(img_t)
            fake_HDR_images.append(img_t)

        fake_HDR_seq = torch.stack(fake_HDR_images, dim=0)  # [b3, c, w, h], b3=3
...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, folder, image_prep, tokenizer, opt):
        """
        Initialize the test dataset object for loading and transforming test data samples
        from specified folder.

        This constructor sets up the paths to the input folder, prepares the transformations
        and tokenizer to be applied on the data, and retrieves the brightness factor.

        Parameters:
        - folder (str): The folder containing the test dataset.
        - image_prep (str): The image preprocessing transformation to apply to each image.
        - tokenizer: The tokenizer used for tokenizing the captions (or prompts).
        - opt: Options object containing exposure information for brightness factor.
        """
        super().__init__()
        self.folder = folder
        self.T = build_transform(image_prep)
        self.tokenizer = tokenizer
        self.caption = "exposure control"
        self.brightness_factor = opt.exposure
        self.img_names = self._collect_img_names()

    def _collect_img_names(self):
        """
        Collect all jpg image names from the specified folder.

        Returns:
        list: A list containing the paths of all images.
        """
        return [os.path.join(self.folder, img) for img in os.listdir(self.folder) if img.endswith('.jpg')]
    ...
    def __getitem__(self, idx):
        """
        Retrieves a dataset item given its index. Each item consists of an input image,
        the brightness factor, and the tokenized caption.

        This method performs the necessary preprocessing on the input image, including
        scaling and normalization, as well as tokenizing the caption using a provided tokenizer.
