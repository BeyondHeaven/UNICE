...
class PairedDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, image_prep, tokenizer):
        ...
        Parameters:
        - dataset_folder (str): The root folder containing the dataset, expected to include sub-folders for different domains (e.g., '+3', '+2', '+1', '0', '-1', '-2', '-3').
    ...
    def _collect_img_names(self):
        """
        Collect all jpg image names from the subfolders.

        Returns:
        list: A list containing the paths of all images.
        """
        img_names = []
        subfolders = [os.path.join(self.dataset_folder, subfolder) for subfolder in os.listdir(self.dataset_folder)]
        for subfolder in subfolders:
            if os.path.isdir(subfolder):
                base_path = os.path.join(subfolder, '0')
                img_names.extend([os.path.join(subfolder, img) for img in os.listdir(base_path) if img.endswith('.jpg')])
        return img_names
    ...
    def __getitem__(self, idx):
        ...
        dict: A dictionary containing the following key-value pairs:
            ...
            scaled to [0, 1].
            - "exposure_factor": the exposure ratio between output image and input image.
        ...
        domains = ['+3', '+2', '+1', '0', '-1', '-2', '-3']
        domain_A, domain_B = random.sample(domains, 2)

        img_A_path = os.path.join(subfolder, domain_A, img_name)
        img_B_path = os.path.join(subfolder, domain_B, img_name)
        ...
        output_t = self.T(img_B)
        output_t = F.to_tensor(output_t)
        output_t = F.normalize(output_t, mean=[0.5], std=[0.5])

        # Convert images to numpy arrays to compute the mean
        img_A_np = np.array(img_A)
        img_B_np = np.array(img_B)
        epsilon = 1e-8
        exposure_factor = np.mean(img_B_np) / (np.mean(img_A_np) + epsilon)
